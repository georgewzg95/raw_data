  ...
  if hierarchy:
    ...
    for line in fin:
      ...
      if start_cell == True and line.rstrip():
        print('INFO:', filepath, line, sep=' ')
        k = line.split()[0]
        if k in cell_name:
          if k not in cell_dict:
            cell_dict[k] = line.split()[1]
            continue
        ...
        if len(x) > 1:
          _cell_width = x[1]
        if _cell_width.isdigit() and _count.isdigit():
          _cell_width = int(_cell_width)
          _count = int(_count)
        else:
          print('ERROR:', filepath, _cell_name, _count, _cell_width, sep=' ')
        if _cell_name not in cell_dict:
          cell_dict[_cell_name] = _cell_width*_count
        else:
          cell_dict[_cell_name] += _cell_width*_count
